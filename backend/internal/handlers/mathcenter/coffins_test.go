...
func TestListCenterCoffins_ArchiveUsesOnlySelectedTerm(t *testing.T) {
	t.Parallel()
	mock, _ := pgxmock.NewPool()
	defer mock.Close()
	r, access, _ := newRouter(t, mock)
	...
	req := authedAdminRequest(t, access, 1, http.MethodGet, "/centers/42/coffins?term_id=70", nil)
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
	if rr.Code != http.StatusOK {
		t.Fatalf("got %d, want 200; body=%s", rr.Code, rr.Body.String())
	...
	if len(resp) != 1 || resp[0]["term_id"] != float64(70) {
		t.Errorf("unexpected archive coffins list: %v", resp)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("unfulfilled expectations: %v", err)
	}
