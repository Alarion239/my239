...
func TestValidateLikbezFields(t *testing.T) {
	date, message := validateLikbezFields(3, 12, "Графы", "2026-07-23", "")
	if message != "" || !date.Valid {
		t.Fatalf("valid payload rejected: date=%+v message=%q", date, message)
	}
	date, message = validateLikbezCreateFields(3, "Черновик Ликбеза", "2026-07-23", "")
	if message != "" || !date.Valid {
		t.Fatalf("valid placeholder rejected: date=%+v message=%q", date, message)
	}
