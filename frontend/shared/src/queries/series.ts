...
// --- Series-level «Разбор» (official solutions) ------------------------------

// useSeriesSolutionTex fetches the разбор LaTeX (gated server-side: teachers
// always, students only after due_at). Loaded on demand via `enabled`.
export function useSeriesSolutionTex(seriesId: number, enabled: boolean) {
  const client = useApiClient()
  return useQuery<{ tex: string }>({
    queryKey: queryKeys.seriesSolutionTex(seriesId),
    queryFn: () =>
      client.request<{ tex: string }>(
        '/mathcenter/series/' + seriesId + '/solution/tex',
      ),
    enabled: enabled && seriesId > 0,
  })
}

// usePutSeriesSolutionTex stores/replaces the разбор LaTeX (teacher-only).
export function usePutSeriesSolutionTex(seriesId: number) {
  const client = useApiClient()
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (tex: string) =>
    ...
    onSuccess: (updated) => invalidateSeries(qc, updated, seriesId),
  })
}

// useDeleteSeriesSolutionTex clears the разбор LaTeX (teacher-only).
export function useDeleteSeriesSolutionTex(seriesId: number) {
  const client = useApiClient()
  const qc = useQueryClient()
  return useMutation({
    mutationFn: () =>
...
// useUploadSeriesSolutionPdf runs the presigned-PUT handshake for the разбор PDF.
export function useUploadSeriesSolutionPdf(seriesId: number) {
  const client = useApiClient()
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async (file: Blob): Promise<Series> => {
      const { object_key, upload_url } = await client.request<PdfUploadURL>(
        '/mathcenter/series/' + seriesId + '/solution/pdf/upload-url',
        { method: 'POST' },
      )
      const put = await fetch(upload_url, {
        method: 'PUT',
      ...
      return client.request<Series>(
        '/mathcenter/series/' + seriesId + '/solution/pdf/publish',
        { method: 'POST', body: { object_key } },
      )
    },
    onSuccess: (updated) => invalidateSeries(qc, updated, seriesId),
...
// useSetSeriesSolutionLink sets (or clears, with "") the external/video link.
export function useSetSeriesSolutionLink(seriesId: number) {
  const client = useApiClient()
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (link: string) =>
      client.request<Series>(
        '/mathcenter/series/' + seriesId + '/solution/link',
...
function invalidateSeries(
  ...
  qc.invalidateQueries({ queryKey: queryKeys.series(seriesId) })
  qc.invalidateQueries({ queryKey: queryKeys.seriesSolutionTex(seriesId) })
  qc.invalidateQueries({ queryKey: queryKeys.seriesList(updated.math_center_id) })
}
