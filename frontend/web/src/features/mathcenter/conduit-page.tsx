...
function ConduitTable({ data }: { data: CenterGridResponse }) {
  ...
  for (const s of data.series) {
    s.columns.forEach((col, i) =>
      cols.push({ col, seriesId: s.series_id, firstInSeries: i === 0 }),
    )
  }
  const students = data.groups.flatMap((g) =>
    g.students.map((st) => ({ ...st, groupName: g.name })),
  )
        ...
        <table className="border-collapse text-sm [&_td]:border [&_td]:border-line [&_th]:border [&_th]:border-line">
          <thead>
            ...
            <tr>
              <th
                rowSpan={2}
                className="sticky left-0 top-0 z-30 min-w-44 bg-surface-muted px-3 py-2 text-left font-medium text-ink"
              >
                Ученик
              </th>
              {data.series.map((s) => (
              ...
              <th
                rowSpan={2}
                className="sticky right-0 top-0 z-30 bg-surface-muted px-3 py-2 text-center font-medium text-ink"
              >
                Решено
              </th>
            </tr>
            ...
            <tr>
              {cols.map(({ col, firstInSeries }) => (
                <th
                  ...
                  className={cn(
                    'sticky top-9 z-10 min-w-9 bg-surface-muted px-1.5 py-1 text-center text-xs font-medium text-muted',
          ...
          <tbody>
              ...
              <Fragment key={g.group_id}>
                <tr>
                  <td
                    ...
                    className="sticky left-0 bg-surface-muted/60 px-3 py-1 text-xs font-medium text-muted"
                  >
                    Группа {g.name}
                  </td>
                </tr>
            ...
            {/* Column totals: people who solved each problem. */}
            <tr>
              <td className="sticky left-0 z-10 bg-surface-muted px-3 py-1.5 font-medium text-ink">
                Решили
              </td>
              {cols.map(({ col, firstInSeries }) => (
                <td
                  key={col.subproblem_id}
                  className={cn(
                    'bg-surface-muted px-1.5 py-1.5 text-center font-medium text-ink',
                    firstInSeries && divider,
                  )}
                >
              ...
              <td className="sticky right-0 z-10 bg-surface-muted px-3 py-1.5 text-center font-medium text-ink">
                {grandTotal}
              </td>
            </tr>
          </tbody>
